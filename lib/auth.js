...
function getPool() {
    if (!pool) {
        ...
        if (!process.env.DATABASE_URL) {
            ...
            logDatabaseError('database configuration', error);
            throw error;
        }
        let databaseUrl;
        try {
            ...
            logDatabaseError('DATABASE_URL parsing', error);
            throw error;
        }
        console.error('[database] target', {
            host: databaseUrl.hostname,
            database: decodeURIComponent(databaseUrl.pathname.slice(1)) || '(default database)'
        });
        const sslMode = databaseUrl.searchParams.get('ssl-mode');
        const requiresSsl = sslMode && sslMode.toUpperCase() === 'REQUIRED' ||
            databaseUrl.hostname.endsWith('.aivencloud.com');
...
function isValidCustomId(customId) {
    return /^[A-Za-z0-9_.-]{3,50}$/.test(customId);
}
...
async function createSession(userId, res, database = getPool()) {
    ...
    try {
        ...
        logDatabaseError('register auth_sessions INSERT', error);
        throw error;
    }
...
module.exports = { createSession, getAuthenticatedUser, getPool, isValidCustomId, isValidEmail, logDatabaseError, normalizeCustomId, normalizeEmail, readJson, sendJson, validatePreferences };